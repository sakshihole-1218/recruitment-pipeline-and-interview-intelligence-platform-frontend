...
import {
  ...
  type CandidateSkillProficiencyLevel,
} from "@/features/candidates/types/candidates.types";

const DEFAULT_PROFICIENCY: CandidateSkillProficiencyLevel = "INTERMEDIATE";

function toOptionLabel(skill: SkillResponse) {
  return skill.code ? `${skill.name} (${skill.code})` : skill.name;
}

export interface CandidateSkillsEditorProps {
  options: SkillResponse[];
  value: CandidateSkillInput[];
  onChange: (value: CandidateSkillInput[]) => void;
  disabled?: boolean;
}
...
export function CandidateSkillsEditor({
  ...
  const updateSkill = (skillId: string, patch: Partial<CandidateSkillInput>) => {
    onChange(value.map((v) => (v.skill_id === skillId ? { ...v, ...patch } : v)));
  };
      ...
      <CardContent sx={{ p: 2.5 }}>
        <Stack spacing={1.5}>
          <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}>
            <Box>
              <Typography sx={{ fontWeight: 800 }}>Skills</Typography>
              <Typography variant="body2" color="text.secondary">
                Add skills and mark primary strengths.
              </Typography>
            </Box>
            <Chip
              size="small"
          ...
          <Autocomplete
            ...
            onChange={(_e, nextSelected) => {
              ...
              const additions: CandidateSkillInput[] = nextSelected
          ...
          <Stack spacing={1.5}>
            {value.map((v) => {
              ...
              const title = skill?.name ?? "Selected skill";
                    ...
                    <Stack spacing={1.25}>
                      ...
                      <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
                        <TextField
                          select
                          label="Proficiency"
                          value={v.proficiency_level ?? DEFAULT_PROFICIENCY}
                          onChange={(e) =>
                            updateSkill(v.skill_id, {
                              proficiency_level: e.target.value as CandidateSkillProficiencyLevel,
                            })
                          }
                          disabled={disabled}
                          sx={{ minWidth: 200 }}
                        >
                          {CANDIDATE_SKILL_PROFICIENCY_LEVELS.map((lvl) => (
                            <MenuItem key={lvl} value={lvl}>
                              {CANDIDATE_SKILL_PROFICIENCY_LABELS[lvl]}
                            </MenuItem>
                          ))}
                        </TextField>
                        ...
                        <TextField
                          ...
                          onChange={(e) => {
                            ...
                            updateSkill(v.skill_id, {
                              years_of_experience: raw === "" ? undefined : Number(raw),
                            });
                          }}
                          disabled={disabled}
                          sx={{ minWidth: 200 }}
                        ...
                        <FormControlLabel
                          ...
                          control={
                            <Switch
                              checked={v.is_primary ?? false}
                              onChange={(_e, checked) =>
                                updateSkill(v.skill_id, { is_primary: checked })
                              }
                              disabled={disabled}
                              color="success"
                            />
                          ...
                          label={
                            <Typography sx={{ fontWeight: 700 }}>
                              {v.is_primary ? "Primary" : "Secondary"}
                            </Typography>
                          }
                        />
                      </Stack>
